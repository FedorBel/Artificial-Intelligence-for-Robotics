...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function
        ...
        if (abs(beta) > 0.001):
            ...
            tetta_prime = (self.orientation + beta)
        else:
            x_prime = self.x + d * cos(self.orientation)
            y_prime = self.y + d * sin(self.orientation)
            tetta_prime = (self.orientation + beta)
            
        x_prime += random.gauss(0.0, self.distance_noise)
        y_prime += random.gauss(0.0, self.distance_noise)
        tetta_prime += random.gauss(0.0, self.steering_noise)
        tetta_prime %= (2*pi)
        
        result = robot(self.length)
        result.set(x_prime, y_prime, tetta_prime)
    ...
    def sense(self, flag = 1): #do not change the name of this function
        ...
        for i in landmarks:
            bearing = (atan2(i[0] - self.y, i[1] - self.x) - self.orientation) % (2*pi)
            if flag:
